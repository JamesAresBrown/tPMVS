...
void CfindMatch::init(const Soption& option) {
  m_timages = option.m_timages; // 目标图像
  ...
  m_tnum = (int)m_timages.size(); // 目标图像数量
  m_num = (int)m_images.size(); // 总图像数量
  
  m_prefix = option.m_prefix;
  m_level = option.m_level; // 金字塔
  m_csize = option.m_csize; // 控制重建的密度
  m_nccThreshold = option.m_threshold; // 如果其相关的光度一致性测量高于此阈值，则补丁重建被视为成功并被保留
  m_wsize = option.m_wsize; // 从每个图像中采样wsize x wsize像素颜色以计算光度一致性分数。例如，当wsize =7 时，在每张图像中采样 7x7=49 像素颜色。增加该值会导致更稳定的重建，但程序会变慢。
  m_minImageNumThreshold = option.m_minImageNum; // 每个 3D 点必须至少在minImageNum 图像中可见才能重建
  m_CPU = option.m_CPU;
  ...
  // We set m_level + 3, to use multi-resolutional texture grabbing 设置 m_level + 3，使用多分辨率纹理抓取
  ...
  // Detect features if not yet done 检测功能（如果尚未完成）
  CdetectFeatures df;
  const int fcsize = 16;
  df.run(m_pss, m_num, fcsize, m_level, m_CPU);  // 完成角点检测与斑点检测
  
  // Initialize each core member. m_po should be first 初始化每个核心成员。 m_po 应该是第一个
  m_pos.init();
...
void CfindMatch::run(void) {
  ...
  m_pos.collectPatches();
    char buffer[1024];
    sprintf(buffer, "%scatch/seedRunEnd.ply", m_prefix.c_str());
    m_pos.writePLY(m_pos.m_ppatches, buffer);
  //----------------------------------------------------------------------
  // Expansion
  const int TIME = 3;
