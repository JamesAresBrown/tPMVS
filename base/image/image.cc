...
// 完成图像的初始化，主要操作是1.载入图像文件名2.设置图像金字塔最大层数
...
// 重载的初始化函数，增加了指定带有边缘（纹理）的区域的图像名称
...
void Cimage::alloc(const int fast, const int filter) {
  ...
  if (readAnyImage(m_name, m_images[0], m_widths[0], m_heights[0], fast) == 0) {
    cerr << "Unsupported iamge format found. Stop allocation: " // 发现不支持的图像格式。 停止分配：
	 << m_name << endl;
    return;
  }
  ...
  if (fast) // fast我的理解是快？如果追求快速的话，在这里图像内存分配的工作就完成了，不再给图像其他要素分配
    return;
  //----------------------------------------------------------------------
  // 蒙板空间分配
  if (!m_mname.empty()) {
                        ...
                        int& width, int& height, const int fast)
{
  ...
  try
  {
    cimage.load(file.c_str());	// CImg determines the file type by extension     CImg 通过扩展名确定文件类型
    if(!cimage.is_empty())
    {
      if(cimage.spectrum() < 3)// .spectrum()函数用来返回通道数
      {
        cerr << "Unsufficient components (not a color image). Component num is " << cimage.spectrum() << endl; // 组件不足（不是彩色图像）。 组件编号为
        return 1;
      }
      width = cimage.width();
      ...
      // CImg在内部以不同的顺序保存图像，所以我们需要在这里重新排序
      int i = 0;
