...
class Cimage {
 public:
  ...
  // 分配和释放内存，在没有分配内存时调用getColor/getMask也会调用这个函数
  void alloc(const int fast = 0, const int filter = 0);
  // free memory 释放内存
  void free(void);
  // free memory below the specified level 低于指定级别的空闲内存
 ...
 protected:
  ...
  //----------------------------------------------------------------------
  // Variables updated at every alloc/free 每次分配/释放时更新变量
  //----------------------------------------------------------------------  
  // 0: nothing allocated
  // 1: width/height allocated
  // 2: memory allocated 已经分配内存
  int m_alloc;
  // a pyramid of images 图像金字塔
  std::vector<std::vector<unsigned char> > m_images;
  // a pyramid of masks 蒙板金字塔
  ...
  //----------------------------------------------------------------------  
  // a name of an image 图像名称
  std::string m_name;
  // a name of a mask image 蒙板图像名称
  std::string m_mname;
